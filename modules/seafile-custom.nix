...
with lib;
let
  cfg = config.services.seafile-custom;
  ...
  ccnetDir = "${seafRoot}/ssd/ccnet";
  dataDir = "${seafRoot}/data";
  seahubDir = "${seafRoot}/ssd/seahub";

in {

  ###### Interface

  options.services.seafile-custom = {
      ...
      seaf-server = {
        ...
        preStart = ''
          if [ ! -f "${seafRoot}/server-setup" ]; then
              ...
              mkdir -p ${seafRoot}/ssd/seafile
              touch ${seafRoot}/ssd/seafile/seafile.db
              ln -s ${seafRoot}/ssd/seafile/seafile.db ${dataDir}/seafile.db
